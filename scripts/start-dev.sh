...
# Usage:
#   ./scripts/start-dev.sh [--rebuild] [--fresh] [--no-frontend] [--no-db] [--no-wait] [--env-stub]
# Flags:
...
echo "[start-dev] Using compose file: $COMPOSE_FILE"

# --- Pre-flight checks -------------------------------------------------------
if [[ ! -f "$COMPOSE_FILE" ]]; then
  echo "[start-dev] ERROR: docker-compose.yml not found at $COMPOSE_FILE" >&2
  exit 1
fi
...
if [[ ! -f "$ENV_FILE" ]]; then
  if [[ $ENV_STUB -eq 1 ]]; then
    cat > "$ENV_FILE" <<'STUB'
# Minimal Numzy dev .env (generated by start-dev)
...
STUB
    echo "[start-dev] Created .env stub at $ENV_FILE"
  else
    echo "[start-dev] WARNING: .env not found (pass --env-stub to generate a stub)" >&2
  fi
fi
...
if [[ $NO_DB -eq 1 ]]; then
  if [[ -z "${DATABASE_URL:-}" ]]; then
    if ! grep -q '^DATABASE_URL=' "$ENV_FILE" 2>/dev/null; then
      echo "[start-dev] WARNING: --no-db used but DATABASE_URL not set; API may fail if it expects Postgres." >&2
    fi
  fi
fi

if ! command -v curl >/dev/null 2>&1; then
  echo "[start-dev] WARNING: curl not found; health waits will rely on nc only." >&2
fi
...
wait_for_port() {
  ...
  echo "[start-dev] Waiting for $label ($host:$port) ..."
  ...
  echo "[start-dev] WARNING: Timeout waiting for $label ($host:$port)" >&2
  return 1
}
...
cleanup() {
  if [[ -f "$FRONTEND_PID_FILE" ]]; then
    ...
    if [[ -n "$pid" ]] && kill -0 "$pid" 2>/dev/null; then
      echo "[start-dev] Stopping frontend dev (PID $pid)"
...
if [[ $FRESH -eq 1 ]]; then
  echo "[start-dev] Fresh start: docker compose down -v"
...
echo "[start-dev] Bringing up backend services (${profiles[*]})"
...
if [[ $NO_FE -eq 0 ]]; then
  echo "[start-dev] Starting frontend dev server (pnpm dev)"
  if command -v pnpm >/dev/null 2>&1; then
    PKG_MGR=pnpm
  elif command -v npm >/dev/null 2>&1; then
    PKG_MGR=npm
  else
    echo "[start-dev] ERROR: Neither pnpm nor npm found in PATH" >&2
    exit 1
  fi
  ...
  echo "[start-dev] Frontend started. Logs: frontend/frontend-dev.log, PID: $(cat "$FRONTEND_DIR/frontend-dev.pid" 2>/dev/null || echo 'n/a')"
fi

cat <<EOF

[start-dev] Dev environment is up:
