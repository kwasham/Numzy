...
export default function Loading() {
	return (
		<>
			...
			<style>{`
				@keyframes nmzy-shimmer { 0% { background-position: 200% 0; } 100% { background-position: -200% 0; } }
				.nmzy-shimmer {
					background: linear-gradient(90deg, #e5e7eb 0%, #f3f4f6 20%, #e5e7eb 40%);
					background-size: 200% 100%;
					animation: nmzy-shimmer 1.4s ease-in-out infinite;
				}
				@media (prefers-reduced-motion: reduce) {
					.nmzy-shimmer { animation: none; background-position: 0 0; }
				}
			`}</style>
		</>
	);
