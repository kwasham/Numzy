...
//                             ≈ {new Intl.NumberFormat("en-US", { style: "currency", currency, maximumFractionDigits: 2 }).format(monthlyReference)} / month
...
// // PlanIcon maps a plan ID to an icon.  Adds support for the enterprise tier by
// // reusing the Business icon.  Update this mapping to use distinct icons as
// // desired.
// function PlanIcon({ name }) {
//     switch (name) {
//         case "free": {
	...
	return (
		...
		>
			...
			{action ?? (
				...
				>
					{id === "business" ? "Contact us" : selected ? "Selected" : "Select"}
				</Button>
			)}
		</Card>
	);
