...
import { RECEIPT_CATEGORY_MAP, subcategoriesForCategory } from "@/components/dashboard/receipts/categories";
...
export function ReceiptsFilters({ filters = {}, onFiltersApplied, onFiltersCleared, sortDir = "desc", view }) {
	...
	const updateSearchParams = React.useCallback(
		(newFilters) => {
			...
			if (sortDir === "asc") searchParams.set("sortDir", sortDir);
			...
			if (newFilters.startDate) searchParams.set("startDate", newFilters.startDate);
			if (newFilters.endDate) searchParams.set("endDate", newFilters.endDate);
		...
		<form onSubmit={handleSubmit(handleApplyFilters)}>
			<Stack spacing={3}>
				...
				<Controller
					...
					name="subcategory"
					render={({ field }) => (
						<FormControl error={Boolean(errors.subcategory)} fullWidth>
							<InputLabel>Sub-category</InputLabel>
							<Select {...field}>
								<Option value="">All</Option>
								{subcategoriesForCategory(watch("category")).map((sub) => (
				...
				<Controller
					...
					render={({ field }) => (
						<FormControl error={Boolean(errors.id)}>
							<InputLabel>Receipt ID</InputLabel>
							<OutlinedInput {...field} />
						</FormControl>
					)}
				/>
				...
				<Controller
					...
					name="merchant"
					render={({ field }) => (
						<FormControl error={Boolean(errors.merchant)}>
